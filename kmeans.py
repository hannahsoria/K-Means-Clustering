...
class KMeans:
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False):
        ...
        verbose: boolean. Print out debug information if set to True.
        '''
        lowest = self.inertia # lowest inertia 

        for i in range(1, k + 1, 1):
            for j in range(n_iter):
                curr_inertia,_ = self.cluster(i) # get the inertia
                if curr_inertia < lowest: # if there is a new lowest
                    lowest = curr_inertia # set the inerta and the corresponding variables
    ...
    def update_centroids(self, k, data_centroid_labels, prev_centroids):
        ...
        centroid_diff = prev_centroids.copy()
        for i in range(k):
            cluster = self.data[data_centroid_labels == i,:] # get the data in the cluster
            if cluster.size == 0:
                rand_int = np.random.rand(1,2) # set random centorid if there is no cluster
                means = rand_int
            else:
                means = np.mean(cluster,axis=0) # get the mean of the cluster
            new_centroids[i] = means # set the new centroid
        centroid_diff = new_centroids - prev_centroids # the difference of the centroid
